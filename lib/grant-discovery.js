...
/**
 * Discover grants from all enabled sources, sorted by region priority (NL first)
 */
export async function discoverGrants(options = {}) {
  const {
    maxSources = 10,
    maxGrantsPerSource = 20,
    region = null, // null = all regions in priority order
  } = options

  let enabledSources
  if (region) {
    enabledSources = getSourcesByRegion(region).slice(0, maxSources)
  } else {
    enabledSources = getSortedSources().slice(0, maxSources)
  }
  ...
  for (let i = 0; i < enabledSources.length; i += BATCH_SIZE) {
    ...
    for (const result of batchResults) {
      if (result.success) {
        ...
        if (!results.byRegion[result.region]) {
          results.byRegion[result.region] = { sourcesScraped: 0, grantsFound: 0 }
        }
        results.byRegion[result.region].sourcesScraped++
        results.byRegion[result.region].grantsFound += result.listings?.length || 0
...
/**
 * Discover grants region by region, NL first, then EU, then rest.
 * Useful for the weekly cron to prioritize closer/more relevant sources.
 */
export async function discoverByRegionPriority(options = {}) {
  const {
    maxSourcesPerRegion = 20,
  ...
  for (const region of regionPriority) {
    ...
    if (regionSources.length === 0) continue

    const regionResult = await discoverGrants({
      maxSources: maxSourcesPerRegion,
      maxGrantsPerSource,
      region,
    })

    allResults.totalSourcesScraped += regionResult.sourcesScraped
    allResults.totalGrantsFound += regionResult.grantsFound
    allResults.relevantGrants.push(...regionResult.relevantGrants)
    allResults.errors.push(...regionResult.errors)
    allResults.regionBreakdown[region] = {
      sourcesScraped: regionResult.sourcesScraped,
      grantsFound: regionResult.grantsFound,
      relevantGrants: regionResult.relevantGrants.length,
    }
  }

  allResults.relevantGrants.sort((a, b) => b.titleRelevanceScore - a.titleRelevanceScore)
